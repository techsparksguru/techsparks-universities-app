const fieldRep = (fieldname) => (fieldname[0].toUpperCase() + fieldname.slice(1)).split('_').join(' ')

const get_code = (fieldname) => {
  return `if (invalidValues.includes(this.payload.${fieldname})) {
        proceed = false;
        this.errors.${fieldname} = "${fieldRep(fieldname)} is required"
    }
    `
}

const get_code2 = (fieldname) => {
  return `<a-row>
    <a-col :span="24">
      <a-form-item
        label="${fieldRep(fieldname)}"
        :labelCol="{ span: 4 }"
        :wrapperCol="{ span: 12 }"
        :validate-status="errors.${fieldname}?'error':'success'"
        :help="errors.${fieldname}"
      >
        <a-input v-model="payload.${fieldname}" />
      </a-form-item>
    </a-col>
  </a-row>
  `
}
const get_code3 = (fieldname) => {
  return `
    {
        title: "${fieldRep(fieldname)}",
        width: 200,
        dataIndex: "${fieldname}",
        key: "${fieldname}"
    },`
...
var code = ['location_details',
  ...
  'hot',
  'public',
].map(fieldname => get_code3(fieldname)).join('')
...
var jsont = `"${text.replace(/(\r\n|\n|\r)/gm,",")}"`
var json = `[${text.replace(/(\r\n|\n|\r|,)/gm,'","')}]`
const fs = require('fs');
fs.writeFile('skills.json', json, (err) => {
